...
function fillOneDay() {
  ...
  deleteAllUncompletedTasks()
    .then(() => {
      ...
      appendCompletedTasksToSheet(regulars["completed"], "day")
        .then(() => {
...
function addTask(title, notes, tasklistname, attempt = 0) {
  ...
  var tasklistsids = {
    "Мои задачи": "MTg0MjQ5MzM2NzAyMTUxNzY1MzM6MDow",
    Утро: "MGwzVWxGbVhmQTdYcDh5Rw",
    Вечер: "VV8zZUhHREhtR2FnZTNZcQ",
    t: "SmJXaGk4UVFaWTdKaC1OWQ",
    y: "a2l1dUkzLTFYWjkyLVFwRA",
    b: "RXZJdHhxczFTVHpPaVAwdw",
  };
...
//   const tasklistId = list; // Замените на ваш список задач
//   const tasks = await getNotcompletedTasks(tasklistId); // Предполагается, что у вас есть функция для получения задач
//   for (const value of tasks) {
//       console.log(value['id']);
//       try {
//           await deleteTask(tasklistId, value['id']);
//       } catch (error) {
//           console.error(`Ошибка при удалении задачи (ID: ${value['id']}):`, error);
//       }
//   }
// }})();
// }
...
//=========================================================================
function getAllCompletedTasks(days) {
  const lists = {
    "Мои задачи": "MTg0MjQ5MzM2NzAyMTUxNzY1MzM6MDow",
    Утро: "MGwzVWxGbVhmQTdYcDh5Rw",
    Вечер: "VV8zZUhHREhtR2FnZTNZcQ",
...
function getAllCompletedAndFormattedTasks() {
  return getAllCompletedTasks().then((alltasks) => {
...
async function rewriteTaskWithRetry(
  ...
  var spreadsheetId = "1mOZUs3quCz6WS4gTQk0FXyFxRDUsPKThR7iLN8TR3bQ";
...
async function getTr_table(byrange = false) {
  var values = await GoogleSheets.Spreadsheets.Values.get(
    "1mOZUs3quCz6WS4gTQk0FXyFxRDUsPKThR7iLN8TR3bQ",
    (range = "tr_table"),
  ).values;
 ...
 */
// async function appendCompletedTasksToSheet(rules, period) {
//   var period=period??'push'
//   var rules=rules??checkConfigForRegulars()['completed']
//   const spreadsheetId = "1mOZUs3quCz6WS4gTQk0FXyFxRDUsPKThR7iLN8TR3bQ";
//   try {
